...
-- | This example implements shows how to use kernmantle-caching
...
type a ~~> b = forall m. (MonadIO m) =>
  AnyRopeWith '[ '("options", GetOpt)
               , '("files", FileAccess) ]
              '[Arrow, HasKleisli m, ProvidesCaching] a b
...
pipeline = proc () -> do
  ...
  caching "computation" computation -< (name, lastname, age, cnt)
  where
...
main = do
    let Cayley pipelineParser =
          pipeline & loosen
            ...
            & untwine
    pipelineReader <- execParser $ info (helper <*> pipelineParser) $
