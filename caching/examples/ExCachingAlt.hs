...
-- | In this example, we show a different caching method than the one presented
-- in kernmantle-caching library and ExCaching. Instead of a class, we use an
-- effect that wraps a Rope.
...
data Cached a b where
  CachedOp  :: CS.CacherM IO a b -> (a ~~> b) -> Cached a b
...
pipeline = proc () -> do
  ...
  where
    ...
    -- cacher.
    cachedOp ::
      (CH.ContentHashable IO a, Store b) =>
      Int -> (a ~~> b) -> (a ~~> b)
